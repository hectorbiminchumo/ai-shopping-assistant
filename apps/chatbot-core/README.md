...
src/
  ...
  image/          # Feature 2: image-based search (voyage-multimodal-3 embeddings)
...
### Embedding model — `voyage-multimodal-3` (512d)

We use **Voyage AI `voyage-multimodal-3`**, configured to **512 dimensions** (`output_dimension: 512`), for both product images (offline, at ingestion) and the user's uploaded query image (online). It must be the same model on both sides — a query embedding is only comparable to product embeddings generated by the identical model, exactly like the `voyage-3` rule for text.

**Why Voyage multimodal instead of CLIP** (the original ticket named CLIP via Replicate):

| Factor | `voyage-multimodal-3` | CLIP ViT-B/32 (Replicate) |
|--------|-----------------------|---------------------------|
| Account / SDK | Already integrated — same `voyageai` client and API key used for text | New Replicate integration, extra API key, cold starts |
...
| Schema fit | 512d output fits the existing `image_embedding vector(512)` column | 512d, also fits |
...
> **Model consistency rule:** never mix embedding models within a vector column. All rows in `image_embedding` must come from `voyage-multimodal-3 @512d`, and query images must use the same. Re-embedding the catalog is required if the model or dimension ever changes.
...
| `image_embedding` | `vector(512)` | `voyage-multimodal-3 @512d` (image) | **0 / 100** — filled by W4 Ticket 3 |
