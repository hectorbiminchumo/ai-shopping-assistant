...
function buildFromProducts(products: HttpTypes.StoreProduct[]): LookItem[] {
  return products.slice(0, 10).map((p, i) => ({
    ...
    src:
      p.thumbnail ??
      `https://placehold.co/360x480/${STATIC_ITEMS[i % STATIC_ITEMS.length].color}/f4f3f0?text=${encodeURIComponent(p.title)}`,
  }))
}

function buildStatic(): LookItem[] {
...
export default async function Lookbook({ regionId }: { regionId: string }) {
  ...
  try {
    const { response } = await listProducts({
      ...
      queryParams: {
        limit: 10,
        fields: "id,title,handle,thumbnail,collection,categories",
      },
    })
    if (response.products?.length) {
      items = buildFromProducts(response.products)
    }
  } catch {
    /* fallback */
  }

  if (!items.length) items = buildStatic()
